...
def test_brat_doc_write(text, lines, sentences, expected_lines):
    """Test that an annotated document in the BRAT format is written correctly."""
    nlp = blank("es")
    brat_spans = [BratSpan.from_bytes(line.encode("utf-8")) for line in lines]
    ...
    for write_call, expected_line in zip_longest(write_calls, expected_lines):
